new Vue({
    ...
    methods: {
      ...
      calculate: function(){
        if(this.salary > 0) {
          Number(this.hourlyPrice) = ((this.salary) / ((this.working) * (this.time)));
      ...
      mainResult: function(){
        if(this.input > 0) {
          Number(this.timeToAchieveResult) = (this.input) / (this.hourlyPrice);
      ...
      timeConvert : function(){
        ...
        this.week = Math.floor(this.rem3 / 7);
        this.rem4 = this.rem3 % 7;
